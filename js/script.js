...
"자, 이제 상상력을 조금만 더 발휘해서, 플랫랜드의 정사각형이 자기 자신과 평행하게 '위쪽으로' 움직인다고 생각해 보게.\n뭐라고요? 북쪽으로 말입니까?\n아니, 북쪽이 아닐세. '위쪽'이라니까. 플랫랜드 바깥으로 완전히 벗어나는 방향이지. 만약 사각형이 북쪽으로 움직인다면, 남쪽의 점들은 북쪽의 점들이 있던 자리를 통과해서 움직여야겠지. 하지만 내 말은 그런 게 아니야. 내 말은 자네 안의 모든 점이—자네는 정사각형이니 설명하기 좋군—자네 안의 모든 점, 즉 자네가 '내부'라고 부르는 곳의 모든 점이 공간을 뚫고 위쪽으로 지나간다는 뜻일세. 어떤 점도 다른 점이 있던 자리를 통과하지 않고, 각 점이 저마다의 직선을 그리면서 말이야. 이건 모두 유비에 딱 들어맞는 설명일세. 자네도 분명 이해할 거야.\n나는 끓어오르는 짜증을 억누르며—마음 같아서는 이 방문객에게 무작정 달려들어 그를 공간 밖으로든, 플랫랜드 밖으로든, 어디로든 밀어버려 없애버리고 싶었지만—이렇게 대답했다.\n그래서 당신이 '위쪽'이라는 단어로 지칭하는 그 움직임으로 제가 만들어낸다는 도형의 정체가 뭡니까? 우리 플랫랜드의 언어로 설명할 수 있겠지요?\n아, 물론이지. 아주 명백하고 단순해. 엄밀한 유비에 따른 것이니까. 단, 그 결과물을 '도형'이라고 부르면 안 되고 '입체'라고 불러야 한다네.";

const FONT_SIZE = (Math.random() * 48) + 36;
// const FONT_SIZE = 18;
...
function setInitialPosition() {
  ...
  // [추가] 캔버스 CSS 스타일 설정 (확대 시 흐림 방지)
  // 브라우저 렌더링 방식을 '픽셀 단위'로 고정하여 성능 저하 없이 선명함 유지
  mainCanvas.style.imageRendering = "pixelated";   // 크롬, 엣지 등 표준
  mainCanvas.style.imageRendering = "crisp-edges"; // 파이어폭스 등 일부 호환

  // 오버레이(텍스트/UI)도 선명하게 유지하려면 동일하게 적용
...
function cacheOverlayPatterns() {
  ...
  hPatternCtx.font=OVERLAY_FONT; 
  hPatternCtx.fillStyle=OVERLAY_BG_COLOR;
  hPatternCtx.fillRect(0, 0, hW, hH);
  hPatternCtx.textBaseline="middle"; 
  hPatternCtx.fillStyle=OVERLAY_COLOR;
  ...
  vPatternCtx.font=OVERLAY_FONT; 
  vPatternCtx.fillStyle=OVERLAY_BG_COLOR;
  vPatternCtx.fillRect(0, 0, hW, hH);
  vPatternCtx.textBaseline="middle"; 
  vPatternCtx.fillStyle=OVERLAY_COLOR;
  // vPatternCtx.fillText(VERTICAL_TEXT,0,vH/2);
  vPatternCtx.fillRect(0, hH/2, hW, OVERLAY_FONT_SIZE/2);
  vPatternCtx.fillRect(0, hH/2, hW, -OVERLAY_FONT_SIZE/2);
}
...
function getHitRegion(x,y){
  ...
  if (showOverlayStrings) {
    ...
    if (x >= vTextX - (OVERLAY_FONT_SIZE/2) - DRAG_HIT_BOX && x <= vTextX + (OVERLAY_FONT_SIZE/2) + DRAG_HIT_BOX) return 'vertical';
  }
...
function drawTextToCanvas() {
    ...
    // 1. [변경] 캔버스 크기 설정 전, 텍스트 줄바꿈 계산을 먼저 수행하여 필요한 높이를 구함
    const val = hiddenInput.value;
    
    // 폰트 스타일을 미리 설정해야 measureText가 정확히 작동함
    textCtx.font = FONT_STYLE; 

    // 내용이 변경되었거나 초기화 상태라면 줄바꿈(renderedLines) 다시 계산
    if (val !== cachedTextValue || renderedLines.length === 0) {
        renderedLines = calculateWrappedLines(val);
        cachedTextValue = val;
    }

    // 2. [추가] 필요한 높이 계산 (줄 수 * 줄 높이 + 상하 여백)
    // 기본 크기(1000)보다 내용이 길면 그만큼 늘어남
    const totalTextHeight = (renderedLines.length * LINE_HEIGHT) + (TEXT_PADDING_Y * 2);
    const newHeight = Math.max(INITIAL_SIZE, totalTextHeight);

    // 3. [변경] 계산된 높이(newHeight)를 적용하여 캔버스 리사이징
    // 가로(INITIAL_SIZE)는 유지, 세로는 newHeight 적용
    if (textCanvas.width !== INITIAL_SIZE * dpr || textCanvas.height !== newHeight * dpr) {
        textCanvas.width = INITIAL_SIZE * dpr;
    ...
    // ... (이하 선택 영역 반전 및 커서 그리기 로직은 기존과 동일) ...
